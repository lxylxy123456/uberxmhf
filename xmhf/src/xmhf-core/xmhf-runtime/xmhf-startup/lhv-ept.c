...
u8 lxy[PAGE_SIZE_4K] __attribute__(( section(".bss.palign_data") ));
...
void lhv_remove_ept(VCPU *vcpu)
{
	...
	hptw_get_pmeo(&pmeo, &ept_ctx.ctx, 1, (uintptr_t) lxy);
	HALT_ON_ERRORCOND(pmeo.lvl == 1);
	HALT_ON_ERRORCOND(((pmeo.pme >> 12) << 12) == (uintptr_t) lxy);
	pmeo.pme = 0;
	HALT_ON_ERRORCOND(hptw_insert_pmeo(&ept_ctx.ctx, &pmeo, (uintptr_t) lxy) == 0);
	hptw_get_pmeo(&pmeo, &ept_ctx.ctx, 1, (uintptr_t) lxy);
	HALT_ON_ERRORCOND(pmeo.lvl == 1);
	HALT_ON_ERRORCOND(pmeo.pme == 0);
}
